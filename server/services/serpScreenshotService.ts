  ...
  async captureSearchResults(
    ...
    try {
      ...
      if (record) {
        ...
        console.log('📄 Record content sample:', JSON.stringify(record, null, 2).substring(0, 500));
      }
      
      if (!record || !record.url) {
        // Try to get screenshot from different key
        const screenshotRecord = await store.getRecord({ key: 'screenshot.png' });
        if (screenshotRecord && screenshotRecord.url) {
          const screenshotUrl = screenshotRecord.url;
          console.log(`✅ Screenshot found with alternate key: ${screenshotUrl}`);
        } else {
          console.log('❌ No screenshot found in any storage location');
          throw new Error('Screenshot not generated by Apify - no URL found in storage');
        }
      }
