...
function EnhancedResultsDashboard({ scanResult, restaurantName }: EnhancedResultsDashboardProps) {
  ...
  const sortedKeywords = useMemo(() => {
    return [...keywordData].sort((a, b) => {
      // Special handling for position sorting (0 or null means "not ranked" = worst)
      if (sortColumn === 'position') {
        const aPos = a.position || 999; // Treat 0/null as 999 (worst position)
        const bPos = b.position || 999; // Treat 0/null as 999 (worst position)
        
        if (sortDirection === 'asc') {
          return aPos > bPos ? 1 : -1; // Best to worst (1, 2, 3... 999)
        }
        return bPos > aPos ? 1 : -1; // Worst to best (999, 50, 10... 1)
      }
