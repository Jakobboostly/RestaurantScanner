...
export class AdvancedScannerService {
  ...
  async scanRestaurantAdvanced(
    ...
    // Remove artificial delays - let it run at natural speed
    ...
    try {
      ...
      console.log(`🚀 PHASE 1 START: ${Date.now() - scanStartTime}ms elapsed`);
      onProgress({ progress: 8, status: 'Finding new customers...' });
      
      // Phase 1: Get business profile and photos in parallel
      ...
      try {
        ...
        // Profile analysis removed for faster scanning
      ...
      // Phase 1 complete - no artificial delay
      console.log(`✅ PHASE 1 COMPLETE: ${Date.now() - scanStartTime}ms elapsed`);

      // Phase 2: Analyzing performance (4 seconds)
      console.log(`🚀 PHASE 2 START: ${Date.now() - scanStartTime}ms elapsed`);
      const phase2Start = Date.now();
      onProgress({ progress: 25, status: 'Analyzing competitor gaps...' });
      ...
      // Phase 2 complete - no artificial delay
      console.log(`✅ PHASE 2 COMPLETE: ${Date.now() - scanStartTime}ms elapsed`);

      // Phase 3: Checking search rankings (4 seconds)
      console.log(`🚀 PHASE 3 START: ${Date.now() - scanStartTime}ms elapsed`);
      const phase3Start = Date.now();
      onProgress({ progress: 42, status: 'Checking review reputation...' });
      ...
      if (localPackRankings && localPackRankings.length > 0) {
        ...
        competitiveOpportunityKeywords = competitiveOpportunityKeywords.map(organicKeyword => {
          ...
          const localMatch = localResults.find((local: any) => local.keyword === organicKeyword.keyword);
          
          if (localMatch && localMatch.position > 0) {
            // Use local pack position if it exists and is better than organic
            return {
              ...organicKeyword,
              localPosition: localMatch.position,
              position: localMatch.position, // Prioritize local pack ranking
              source: 'local_pack'
            };
          }
      ...
      if (competitiveOpportunityKeywords.length === 0) {
        console.log('⚠️ No ranking data returned - generating authentic search volume data for fallback');
        ...
        competitiveOpportunityKeywords = fallbackResults;
      }
      ...
      // Enhanced Keyword Discovery: Get actual keywords from restaurant's domain (after competitors are found)
      let enhancedKeywordDiscovery: any[] = [];
      let competitiveGaps: any[] = [];
      
      try {
        console.log('🚀 Enhanced keyword discovery temporarily disabled for stability');
        // Temporarily disable to prevent scan failures - will re-enable after testing
        enhancedKeywordDiscovery = [];
        competitiveGaps = [];
        
        // TODO: Re-enable after testing
        /*
        // Get actual keywords the restaurant's website ranks for
        const siteKeywords = await this.urlRankingService.getKeywordsForSite(
          actualDomain,
          locationData.city,
          locationData.state,
          15 // Top 15 opportunities
        );
        
        if (siteKeywords.length > 0) {
          enhancedKeywordDiscovery = siteKeywords; // Keep all for display
          console.log(`✅ Enhanced keyword discovery: Found ${enhancedKeywordDiscovery.length} keyword opportunities`);
        }
        */
        
      } catch (error) {
        console.log('⚠️ Enhanced keyword discovery failed:', error);
        // Don't let this break the scan - continue with empty arrays
        enhancedKeywordDiscovery = [];
        competitiveGaps = [];
      }
      ...
      // Phase 5 complete - no artificial delay
      ...
      // Phase 6 complete - no artificial delay
  ...
  private analyzeContextualClues(combinedName: string, businessProfile: any): string {
    ...
    const words = combinedName.split(/\s+/);
    
    // Check for compound restaurant names that might indicate cuisine
    const compoundPatterns = [
      { pattern: /kitchen/i, indicators: ['hash', 'waffle', 'pancake', 'breakfast', 'brunch', 'eggs', 'omelet'], cuisine: 'breakfast' },
    ...
    for (const compound of compoundPatterns) {
      if (compound.pattern.test(combinedName)) {
        console.log(`🔍 CUISINE COMPOUND MATCH: "${combinedName}" matches pattern ${compound.pattern.source}`);
        for (const indicator of compound.indicators) {
          if (combinedName.includes(indicator)) {
            console.log(`🔍 CUISINE COMPOUND DETECTED: "${indicator}" + "${compound.pattern.source}" -> ${compound.cuisine}`);
            return compound.cuisine;
          }
        }
        console.log(`🔍 CUISINE COMPOUND: Pattern matched but no indicators found. Indicators: [${compound.indicators.join(', ')}]`);
      }
    }
    ...
    const address = businessProfile?.address || businessProfile?.formatted_address || '';
    for (const regional of regionalPatterns) {
      for (const region of regional.regions) {
        if (address.toLowerCase().includes(region) || combinedName.includes(region)) {
          // Only use regional defaults for very generic names
          if (words.length <= 2 && !combinedName.includes('restaurant')) {
            return regional.defaultCuisine;
          }
        }
      }
    }
  ...
  private async generateEnhancedReviewsAnalysis(businessProfile?: any, placeId?: string, restaurantName?: string): Promise<any> {
    ...
    if (apifyReviews && apifyReviews.success && apifyReviews.data && apifyReviews.data.length > 0) {
      ...
      // Determine restaurant type based on average price or other indicators
      const restaurantContext = this.determineRestaurantContext(restaurantName || businessProfile?.name || 'Restaurant', businessProfile);
      const customerMoodAnalysis = await this.openaiReviewAnalysisService.analyzeCustomerMood(apifyReviews.data, restaurantContext);
    ...
    // Fallback: Use Google Places API reviews (limited to 5 reviews)
      ...
      const restaurantContext = this.determineRestaurantContext(restaurantName || businessProfile?.name || 'Restaurant', businessProfile);
      const customerMoodAnalysis = await this.openaiReviewAnalysisService.analyzeCustomerMood(googleReviews.reviews, restaurantContext);
  ...
  private async analyzeMobilePerformance(domain: string): Promise<any> {
    try {
      ...
      const response = await axios.get('https://www.googleapis.com/pagespeedonline/v5/runPagespeed', {
        ...
        timeout: 8000 // Aggressive timeout for faster scanning
      });
  ...
  // Restaurant Context Determination
  private determineRestaurantContext(restaurantName: string, businessProfile?: any): { name: string; type: 'QSR' | 'FSR'; locationCount: number } {
    const name = restaurantName || 'Restaurant';
    
    // Determine restaurant type based on business indicators
    let type: 'QSR' | 'FSR' = 'FSR'; // Default to Full Service Restaurant
    
    // QSR indicators in name
    const qsrKeywords = ['pizza', 'burger', 'sandwich', 'taco', 'chicken', 'coffee', 'donut', 'bagel', 'sub', 'wrap', 'bowl', 'salad bar', 'fast', 'quick', 'express'];
    const fsrKeywords = ['grill', 'steakhouse', 'bistro', 'tavern', 'bar', 'restaurant', 'dining', 'house', 'cafe', 'kitchen', 'table'];
    
    const nameLower = name.toLowerCase();
    
    // Check for QSR indicators first (more specific)
    if (qsrKeywords.some(keyword => nameLower.includes(keyword))) {
      type = 'QSR';
    }
    
    // Check for common chain indicators
    const qsrChains = ['mcdonald', 'burger king', 'subway', 'kfc', 'taco bell', 'pizza hut', 'domino', 'chipotle', 'starbucks', 'dunkin'];
    if (qsrChains.some(chain => nameLower.includes(chain))) {
      type = 'QSR';
    }
    
    // Price level indicators (if available from business profile)
    if (businessProfile?.priceLevel) {
      // Google price levels: 1 = inexpensive, 2 = moderate, 3 = expensive, 4 = very expensive
      if (businessProfile.priceLevel <= 2) {
        type = 'QSR';
      } else {
        type = 'FSR';
      }
    ...
    // For now, assume single location unless we have other data
    const locationCount = 1;
    
    return { name, type, locationCount };
  }
